# main.py — FINAL & 100% WORKING — BINANCE.US + KRAKEN (December 07, 2025)
...
# ================= ARBITRAGE — XRP/USD (Binance.US) + XRP/USDC (Kraken) =================
@app.get("/arbitrage")
async def arbitrage(email: str = Query(...)):
    ...
    try:
        ...
        # Normalize to USDC price (USD ≈ USDC for arbitrage)
        spread = abs(b_price - k_price) / min(b_price, k_price)
        net = spread - 0.0086  # 0.86% round-trip fees
        roi = max(net * 100.0, 0)

...
async def set_amount(data: dict):
    ...
    try:
        ...
        amount = 0.0
    c.execute("INSERT INTO user_settings (email, trade_amount) VALUES (?, ?) ON CONFLICT(email) DO UPDATE SET trade_amount=excluded.trade_amount", (email, amount))
    conn.commit()
    return {"status": "ok"}
...
async def toggle_auto_trade(data: dict):
    ...
    c.execute("INSERT INTO user_settings (email, auto_trade) VALUES (?, ?) ON CONFLICT(email) DO UPDATE SET auto_trade=excluded.auto_trade", (email, enabled))
    conn.commit()
